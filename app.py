...
def auto_create_projects_from_work_orders():
    ...
    try:
        ...
        for group in company_groups:
            ...
            cursor.execute("SELECT id FROM work_order_projects WHERE title LIKE ?", (f"%{company_name}%",))
            existing_project = cursor.fetchone()
            
            if not existing_project:
                # Create new project
                cursor.execute("""
                    INSERT INTO work_order_projects (title, company_name, total_work_orders)
                    VALUES (?, ?, ?)
                """, (
...
def auto_create_projects_by_title():
    ...
    try:
        ...
        logger.error(f"Error auto-creating projects by title: {e}")
        return jsonify({'error': str(e)}), 500
